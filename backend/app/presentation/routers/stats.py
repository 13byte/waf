...
async def get_monitoring_stats(
    ...
    while current_hour <= end_date:
        ...
        hour_query = db.query(SecurityEvent).filter(
            SecurityEvent.timestamp >= current_hour,
            SecurityEvent.timestamp < next_hour
        )
        hourly_stats.append({
            "hour": current_hour.isoformat(),
            "total": hour_query.count(),
            "blocked": hour_query.filter(SecurityEvent.is_blocked == True).count(),
            "attacks": hour_query.filter(SecurityEvent.is_attack == True).count()
        })
